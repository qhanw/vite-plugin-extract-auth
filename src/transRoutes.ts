...
export default function transRoutes(cfg: Options) {
  ...
  const json: AuthRoute[] = new Function('return ' + jsonText)();
    ...
    return (routes || []).reduce((prev, curr) => {
      ...
      if (children?.length) {
        ...
        if (content) {
          ...
          const collection = scanDir(join(fileDir, 'auth'))?.map((filePath) => {
      ...
      const { path: p, json } = combinePaths(childAuth, key, full.path);
  ...
  // 只有根下的路由具有权限配置
  return parse(json.find((c) => c.path === '/')?.children || []);
}
